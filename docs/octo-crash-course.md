# Octo.nvim Crash Course

Octo.nvim is a powerful plugin that allows you to interact with GitHub issues and pull requests directly from Neovim. Here's a quick guide to get you started:

# Basic Commands

All Octo commands start with :Octo. Here are the most common ones:

## Issues

```ruby
:Octo issue list # List issues in the current repository
:Octo issue create # Create a new issue
:Octo issue edit # Edit issue details
:Octo issue close # Close an issue
:Octo issue reopen # Reopen a closed issue
:Octo issue search # Search for issues
```

## Pull Requests

```ruby
:Octo pr list # List PRs in the current repository
:Octo pr create # Create a new PR
:Octo pr checkout <number> # Checkout a PR locally
:Octo pr diff # Show PR diff
:Octo pr merge # Merge a PR
:Octo pr review start # Start reviewing a PR
```

## Navigation

```ruby
:Octo repo list # List repositories
:Octo gist list # List gists
```

# Working with Issues and PRs

When you open an issue or PR with Octo, you'll get a buffer with the issue/PR details. Based on your configuration, you can use these key mappings:

## Issue Mappings

- `<localleader>ic` - Close issue
- `<localleader>io` - Reopen issue
- `<localleader>il` - List open issues
- `<C-r>` - Reload issue
- `<C-b>` - Open issue in browser
- `<C-y>` - Copy URL to clipboard
- `<localleader>aa` - Add assignee
- `<localleader>ad` - Remove assignee
- `<localleader>la` - Add label
- `<localleader>ld` - Remove label
...
1. Open a PR with :Octo pr list and select one
1. Start a review with <localleader>vs
1. Navigate through changed files in the file panel
1. Add comments with <localleader>ca or suggestions with <localleader>sa
1. Submit your review with <localleader>vs from the diff view
1. Choose to approve, comment, or request changes
...
If you need to see all available commands:

```ruby
:Octo help
```

To see the mappings available in the current buffer:
...
1. Navigate to your git repository in Neovim
1. Run :Octo issue list to see open issues
1. Select an issue to view its details
1. Add a comment with <localleader>ca
1. Close the issue with <localleader>ic if resolved

# Lazy Configuration used for this crash course
